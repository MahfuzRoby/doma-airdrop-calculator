...
deep_sync_progress = {"running": False, "current_week": None, "total_weeks": None,
                       "unique_wallets_so_far": None, "started_at": None}
...
def deep_sync():
    ...
    deep_sync_progress.update(running=True, current_week=None, total_weeks=None,
                               unique_wallets_so_far=None, started_at=now)

    def on_progress(week, total_weeks, unique_so_far):
        deep_sync_progress.update(current_week=week, total_weeks=total_weeks,
                                   unique_wallets_so_far=unique_so_far)

    try:
        items = fetch_full_history(API_KEY, on_progress=on_progress)
    except Exception as e:
