...
from scraper import fetch_all_wallets, get_current_week

# module-level progress tracker for the currently-running deep sync, so the
# frontend/you can poll a lightweight endpoint instead of reading logs
deep_sync_progress = {"running": False, "current_week": None, "total_weeks": None,
                       "unique_wallets_so_far": None, "started_at": None, "failed_weeks": []}
...
def deep_sync():
    """Deep sync: walks every week from 1 to the current week, upserting
    into the DB after EACH week (not just at the end). This means:
      - a timeout/error on one week doesn't wipe out progress from earlier
        weeks in the same run
      - if a week fails even after retries, it's skipped and logged, and the
        run continues with the remaining weeks instead of aborting entirely
      - the daily schedule will naturally retry any skipped week next time
    Slower than the fast sync (roughly 20x the API calls once there are
    ~20 weeks), so this runs on its own daily schedule instead of hourly.
    """
    now_started = datetime.now(timezone.utc).isoformat()
    print(f"[{now_started}] Starting deep sync (all weeks)...")
    deep_sync_progress.update(running=True, current_week=None, total_weeks=None,
                               unique_wallets_so_far=None, started_at=now_started,
                               failed_weeks=[])

    try:
        current_week = get_current_week(API_KEY)
    except Exception as e:
        print("Deep sync failed to determine current week:", e)
        deep_sync_progress.update(running=False)
        with get_conn() as conn:
            conn.execute(
                "INSERT INTO sync_log (synced_at, total_wallets, total_season_points, status, error) "
                "VALUES (?, ?, ?, ?, ?)",
                (now_started, 0, 0, "deep_error", str(e)),
            )
            conn.commit()
        return
    ...
    for week in range(1, current_week + 1):
        try:
            items = fetch_all_wallets(API_KEY, week_number=week, verbose_label=f"week {week}/{current_week}")
        except Exception as e:
            print(f"  [deep sync] week {week}/{current_week} FAILED after retries: {e} - skipping, will retry next run")
            failed_weeks.append(week)
            deep_sync_progress.update(failed_weeks=list(failed_weeks))
            continue
        ...
        for i in items:
            seen_addresses.add(i["walletAddress"].lower())

        print(f"  [deep sync] week {week}/{current_week} saved - {len(items)} wallets this week, "
              f"{len(seen_addresses)} unique wallets so far")
        deep_sync_progress.update(current_week=week, total_weeks=current_week,
                                   unique_wallets_so_far=len(seen_addresses))
    ...
    with get_conn() as conn:
        ...
        status = "deep_ok" if not failed_weeks else "deep_partial"
        error_msg = f"weeks failed: {failed_weeks}" if failed_weeks else None

        conn.execute(
            "INSERT INTO sync_log (synced_at, total_wallets, total_season_points, status, error) "
            "VALUES (?, ?, ?, ?, ?)",
            (now_started, row["c"], row["sp"], status, error_msg),
        )
        conn.execute(
            "INSERT INTO points_history (recorded_at, total_wallets, total_season_points, total_weekly_points) "
            "VALUES (?, ?, ?, ?)",
            (now_started, row["c"], row["sp"], row["wp"]),
        )
        conn.commit()

    deep_sync_progress.update(running=False)
    print(f"Deep sync complete: {row['c']} total wallets tracked, {row['sp']:,.0f} total season points"
          + (f" (weeks {failed_weeks} failed and will retry next run)" if failed_weeks else ""))
...
@app.get("/api/deep-sync-progress")
def get_deep_sync_progress():
    """Poll this while a deep sync is running to see live progress (which
    week it's on) instead of digging through Railway's log viewer."""
    return deep_sync_progress
def status():
    with get_conn() as conn:
...
def deep_sync_now():
    """Manually trigger a deep sync (walks every week 1..current).

    Runs in a background thread rather than blocking this request, since a
    full deep sync can take many minutes - far longer than most reverse
    proxies (including Railway's) will hold an HTTP connection open for.
    Poll /api/status afterward to see when it finishes.
    """
    threading.Thread(target=deep_sync, daemon=True).start()
    return {"status": "started", "type": "deep", "note": "poll /api/status for progress"}
